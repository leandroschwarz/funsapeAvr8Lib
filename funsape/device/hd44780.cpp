...
bool_t Hd44780::init(const Size size_p, const Font font_p)
{
    ...
    this->_gpioControlE->setMode(PinMode::OUTPUT_PUSH_PULL);
    this->_gpioControlRs->low();
    this->_gpioControlRs->setMode(PinMode::OUTPUT_PUSH_PULL);
    if(this->_useBusyFlag) {
        this->_gpioControlRw->low();
        this->_gpioControlRw->setMode(PinMode::OUTPUT_PUSH_PULL);
    }
...
bool_t Hd44780::_waitUntilReady(void)
{
    ...
    this->_gpioDataBus->setMode(PinMode::INPUT_FLOATING);         // Data bus in input mode
    this->_gpioControlRs->low();                                        // LCD in command mode
    ...
    // Restore LCD status
    this->_gpioControlRw->low();                                        // LCD in write mode
    this->_gpioDataBus->setMode(PinMode::OUTPUT_PUSH_PULL);       // Data bus in output mode
